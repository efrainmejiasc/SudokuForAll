...
namespace SudokuDeTodos.Vista
{
    public partial class NewGame : System.Web.UI.Page
    {
        ...
        protected void GuardarNuevoJuejo_Click(object sender, EventArgs e)
        {
            ...
            IniciarArreglos();
            AsociarTxtMatriz(txtSudoku);
            valorIngresado = Game.SetNuevoJuego(txtSudoku, valorIngresado);
            valorInicio = valorIngresado;
            bool resultado = Game.GuardarJuego(pathArchivo, valorIngresado, valorEliminado, valorInicio, valorSolucion);
            if (resultado)
                Response.Redirect("GameAOne.aspx");
        }

        private void IniciarArreglos()
        {
            valorIngresado = Game.IniciarArreglo(valorIngresado);
            valorInicio = valorIngresado;
            valorEliminado = Game.IniciarArreglo(valorEliminado);
            valorSolucion = Game.IniciarArreglo(valorSolucion);
        }
